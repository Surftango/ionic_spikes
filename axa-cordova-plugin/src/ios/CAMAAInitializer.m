...
- (void) setUserFeedback:(CDVInvokedUrlCommand *)command{
    isArgValidString(command, 0);
    NSString *res = [command.arguments objectAtIndex:0];
   [CAMDOReporter setUserFeedback:res];
    successfulCall();

}

- (void) setCrashFeedback:(CDVInvokedUrlCommand *)command{
    isArgValidString(command, 0);
    NSString *res = [command.arguments objectAtIndex:0];
    [CAMDOReporter setCrashFeedback:res];
    successfulCall();
}
